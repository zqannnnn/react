...
const edit: ActionCreator<ThunkAction<void, RootState, void>> = (
  ...
  return (dispatch: Dispatch<RootState>): void => {
    ...
    offerService.edit(offer, offerId).then(
      () => {
        dispatch(success())
...
function getById(id: string) {
  return (dispatch: (action: Action) => void) => {
    ...
    offerService
      .getById(id)
      .then(
        (offer: Offer) => dispatch(success(offer)),
  ...
  function success(offer: Offer) {
    if (offer.images) {
      let images = offer.images.filter(
        image => image.type === offerConsts.IMAGE_TYPE_MEDIE
      )

      let certificates = offer.images.filter(
        image => image.type === offerConsts.IMAGE_TYPE_CERTIFICATE
      )
      offer.images = images
...
const getAll: ActionCreator<ThunkAction<void, RootState, void>> = (option: {
  ...
  return (dispatch: Dispatch<RootState>): void => {
    ...
    offerService
      .getAll(option)
      .then(
        (offers: Array<Offer>) => dispatch(success(offers)),
        (error: string) => dispatch(failure(error))
      )
  }
...
function addComment(id: string, comment: string) {
  return (dispatch: (action: Action) => void) => {
    ...
    offerService.addComment(id, comment).then(
