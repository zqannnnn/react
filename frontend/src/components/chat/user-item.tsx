...
class UserItem extends React.Component<ItemProps, ItemState> {
    ...
    if (props.messages !== undefined) {
      ...
      Object.keys(props.messages).map((key, index) => {
        if (messages[key] == undefined) {
          ...
          if (!isNewMessage && props.messages[key].from == props.userKey)
            isNewMessage = props.messages[key].isNew
        }
      })
      ...
      if (isNewMessage) {
        ...
        if (this.props.socket !== undefined)
          this.props.socket.emit('set-messages-as-old', data)
      }
    }
  }
  scrollBottom() {
    ...
    const that = this
    setTimeout(() => {
      if (that.chatBottom.current != null)
        that.chatBottom.current.scrollIntoView({ behavior: 'smooth' })
    }, 300)
  }
  componentDidMount() {
  ...
  componentDidUpdate(prevProps: any, prevState: any) {
    if (
      ...
      if (scrollBottom) {
        ...
        if (
          this.msgsList.current != null &&
          this.msgsList.current.scrollTop == 0
        )
          this.msgsList.current.scrollTop = 20
      }
    } else {
      if (
        this.state.msgs.length > 0 &&
        this.state.msgs[this.state.msgs.length - 1].isNew
      ) {
        this.scrollBottom()
  ...
  listenScrollEvent(e: any) {
    if (e.target.scrollTop == 0) {
      this.setLoading()
      const data = {
        from: this.props.ownerUserKey,
        to: this.props.userKey,
        createdAt: this.state.msgs[0].createdAt
      }
      this.props.socket.emit('get-previous-messages', data)
    }
  }
        ...
        <Spin spinning={this.state.loading}>
          ...
          >
            {this.state.msgs.map(function(msg, index) {
              if (
                msg.from == that.props.userKey ||
                msg.to == that.props.userKey
              ) {
                let cssClass =
                  msg.from == that.props.userKey ? 'incoming' : 'outcoming'
