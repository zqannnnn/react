...
class Chat extends React.Component<ItemProps, ItemState> {
  constructor(props: ItemProps) {
    ...
    if (
      ...
      this.props.auth.authInfo != undefined &&
      this.props.auth.authInfo.id != undefined
    )
      userId = this.props.auth.authInfo.id
    this.state = {
      users: {},
      socket: undefined,
      messages: {},
  ...
  openChat(userId: string, open = true) {
    ...
    if (users[userId] != undefined) {
      ...
      if (this.state.socket !== undefined)
        this.state.socket.emit('get-user', data)
    }
  ...
  componentWillUpdate() {
    ...
    if (!this.state.active) {
      for (let userKey in this.state.users) {
        if (this.state.users[userKey]['newMsg'])
          this.setState({ active: true, opened: false })
      }
    }
  }
  //TODO: strings to const
  ...
  updateUserMsgs(userKey: string) {
    ...
    if (users[userKey]['panelStatus'] == 'collapsed')
      users[userKey]['newMsg'] = isNewMessage
    this.setState({ users: users })
  }
  onPrivateMsg(msg: any) {
    ...
    for (let userKey in this.state.users) {
      if (
        ...
        (msg.from == userKey || msg.to == userKey)
      ) {
        foundUser = true
  ...
  onPrivateMsgsBatch(msgs: any[]) {
    ...
    for (let userKey in this.state.users) {
      if (
        userKey != this.state.userKey &&
        (msgs[0].from == userKey || msgs[0].to == userKey)
      ) {
        foundUser = true
        this.updateUserMsgs(userKey)
      }
        ...
        <>
          <div id="chat">
            <Collapse
              ...
              accordion
              onChange={this.onOpenChat}
            >
              <Panel
                className={chatCssClass}
                header={
                  <PanelHead
                    onUserItemClose={this.onChatClose}
                    user={{}}
                    userKey="none"
                    text={i18n.t('Chat')}
                    showClose={true}
                    ...
                    if (this.state.userKey != userKey) {
                      ...
                      return (
                        <Panel
                          ...
                          header={
                            <PanelHead
                              onUserItemClose={this.onUserItemClose}
                              user={users[userKey]}
                              userKey={userKey}
                              text=""
                              showClose={true}
                        ...
                        >
                          <UserItem
                            messages={users[userKey]['messages']}
                            userKey={userKey}
                            socket={this.state.socket}
                            ownerUserKey={this.state.userKey}
                          />
                        </Panel>
                      )
                    }
