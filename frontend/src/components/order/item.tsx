...
=======
  ...
  handleFinish = (id: string) => {
    let r = confirm('Are you sure?')
    if (r) this.props.dispatch(orderActionCreators.finish(id))
  }
  triggerCommentInput = () => {
    let value = this.state.commentInputShowing
    this.setState({ commentInputShowing: !value })
  ...
  render() {
    ...
    const { commentInputShowing, comment } = this.state
    return (
      <Col key={order.id} xs={12} sm={11} md={10} lg={9} className="block">
        <div className="boxmain">
          <div className="header">{order.type}</div>
          <div className="desc">
            <span>{order.storage && 'Storage:' + order.storage + ','}</span>
            ...
            <span>
              {order.slaughterSpec &&
                'Slaughter Specificatin:' + order.slaughterSpec + ','}
            </span>
            <span>{order.primalCuts && 'Primal Cut:' + order.primalCuts}</span>
          </div>
          <div className="space-between content">
            <div className="status">
              {order.status == 0
                ? 'Canceled'
                : order.status != orderConsts.ORDER_STATUS_FINISHED
                  ? 'On Sale'
                  : 'Sold'}
            </div>
            {authInfo.isAdmin &&
            order.status != orderConsts.ORDER_STATUS_FINISHED ? (
              <div
                ...
                onClick={() => {
                  if (order.id) this.handleFinish(order.id)
                }}
              >
                Set Sold
              </div>
          ...
          <div className="menu content">
            <Link className="control-btn" to={'/order/' + order.id}>
              Read More
            </Link>
            {authInfo.id == order.userId || authInfo.isAdmin ? (
              <>
                {order.status === orderConsts.ORDER_STATUS_CREATED && (
                  <>
                    ...
                    <div
                      className="control-btn"
                      onClick={() => {
                        if (order.id) this.handleCancell(order.id)
                      }}
                    >
                      Cancel{' '}
            ...
            <div className="input-wr content">
              ...
              <div className="input-btn-wr">
                <i
                  ...
                  aria-hidden="true"
                  onClick={() => {
                    if (order.id) this.sendComment(order.id)
                  }}
                />
              </div>
            </div>
          ) : (
            <div className="comment content">{order.comment}</div>
          )}
        </div>
      </Col>
    )
