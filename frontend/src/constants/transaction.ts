export const consts = {
  CREATE_REQUEST: 'TRANSACTION_CREATE_REQUEST',
  CREATE_SUCCESS: 'TRANSACTION_CREATE_SUCCESS',
  CREATE_FAILURE: 'TRANSACTION_CREATE_FAILURE',

  EDIT_REQUEST: 'TRANSACTION_EDIT_REQUEST',
  EDIT_SUCCESS: 'TRANSACTION_EDIT_SUCCESS',
  EDIT_FAILURE: 'TRANSACTION_EDIT_FAILURE',

  GET_REQUEST: 'TRANSACTION_GET_REQUEST',
  GET_SUCCESS: 'TRANSACTION_GET_SUCCESS',
  GET_FAILURE: 'TRANSACTION_GET_FAILURE',

  GETALL_REQUEST: 'TRANSACTION_GETALL_REQUEST',
  GETALL_SUCCESS: 'TRANSACTION_GETALL_SUCCESS',
  GETALL_FAILURE: 'TRANSACTION_GETALL_FAILURE',

  CANCELL_REQUEST: 'TRANSACTION_CANCELL_REQUEST',
  CANCELL_SUCCESS: 'TRANSACTION_CANCELL_SUCCESS',
  CANCELL_FAILURE: 'TRANSACTION_CANCELL_FAILURE',

  FINISH_REQUEST: 'TRANSACTION_FINISH_REQUEST',
  FINISH_SUCCESS: 'TRANSACTION_FINISH_SUCCESS',
  FINISH_FAILURE: 'TRANSACTION_FINISH_FAILURE',

  UPLOAD_REQUEST: 'TRANSACTION_UPLOAD_REQUEST',
  UPLOAD_SUCCESS: 'TRANSACTION_UPLOAD_SUCCESS',
  UPLOAD_FAILURE: 'TRANSACTION_UPLOAD_FAILURE',

  COMMENT_REQUEST: 'TRANSACTION_COMMENT_REQUEST',
  COMMENT_SUCCESS: 'TRANSACTION_COMMENT_SUCCESS',
  COMMENT_FAILURE: 'TRANSACTION_COMMENT_FAILURE',

  TRANSACTION_TYPE: ['Beef', 'Veal', 'Sheep'],
  TRANSACTION_STATUS_CANCELLED: 0,
  TRANSACTION_STATUS_CREATED: 1,
  TRANSACTION_STATUS_FINISHED: 2,

  IMAGE_TYPE_MEDIE: 1,
  IMAGE_TYPE_CERTIFICATE: 2
}
